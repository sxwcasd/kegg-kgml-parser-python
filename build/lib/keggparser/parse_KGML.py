...
_FILENAME_SEPARATOR_TRANSLATION = str.maketrans({"/": "-", "\\": "-"})


def _safe_filename(name):
    return name.translate(_FILENAME_SEPARATOR_TRANSLATION)


def parse_edges_recursively(substrates: list, products: list):
    """
    helper function to loop through substrates and products
    and adding edges between each of them
    inputs:
        substrates: list of substrates
        products: list of products
    outputs:
        edges: list of tuples with (substrate, product)
    """
    edges = []
    for s in substrates:
        for p in products:
...
#    logging.debug(nodes)
    ...
    for reaction in tree.iter('reaction'):
        ...
        for sub in reaction.iter('substrate'):
            substrates.append(sub.get('name'))
            substrates_id.append(sub.get('id'))

        for prod in reaction.iter('product'):
            products.append(prod.get('name'))
            products_id.append(prod.get('id'))

        reaction_edges = parse_edges_recursively(substrates_id, products_id)
        for e in reaction_edges:
            pathway.add_edge(e[0], e[1])

        pathway.reactions[id] = {'reaction': reaction, 'substrates': substrates, 'products': products}

    return tree, pathway, nodes, genes

def plot_starlike(pathway, save_dir = './plots/'):
    pylab.figure()
    networkx.draw_circular(pathway, labels=pathway.labels)
    pylab.title(pathway.title)
    title = _safe_filename(pathway.title)
    pylab.savefig(save_dir + title + '.png')
    pylab.show()


def plot_original(pathway, save_dir='./plots/'):
    pos = {}
    for n in pathway.nodes():
        pos[n] = pathway.nodes[n]['xy']
    pylab.figure()
    networkx.draw_networkx(pathway, pos, labels=pathway.labels)
    title = _safe_filename(pathway.title)
    pylab.savefig(save_dir + title + '_original_layout.png')
    pylab.show()
